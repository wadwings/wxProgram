...
Component({
  ...
  detached() {
    for (let t in this._keys) Object.prototype.hasOwnProperty.call(this._keys, t) && (eventBus.off("lin-form-blur-" + t), eventBus.off("lin-form-change-" + t))
  },
  data: {
    _this: null
  },
  methods: {
    _init() {
      wx.lin = wx.lin || {}, wx.lin.forms = wx.lin.forms || {}, wx.lin.forms[this.properties.name] = this, wx.lin.initValidateForm = t => {
    ...
    _initItem(t) {
      ...
      if (eventBus.on("lin-form-blur-" + e, t => {
        ...
        }), this._keys[e]) throw new Error("表单项存在重复的name：" + e);
